"""
=============================================================================
  HERO 3D TYPOGRAPHY RENDERER: WHITE STUDIO BACKGROUND
=============================================================================
Renders the 35-second seamless looping video with:
- Aspect Ratio matching viewBox 0 0 2679.8 294 (1920 x 210)
- Pure white background (#ffffff) seamlessly blending into the hero section
- Crisp anti-aliased 3D typography letters
- Exact bounding box matching original hero layout
- Continuous 10-state physical material morphing
- 100% web-compatible H.264 MP4 with +faststart
=============================================================================
"""
...
WIDTH = 1920
HEIGHT = 210  # Exact matching aspect ratio for viewBox 0 0 2679.8 294
FPS = 30
TOTAL_SECONDS = 35
...
def load_and_preprocess_states():
    print("Loading, extracting alpha, and normalizing 10 material target states...")
    processed_rgb = []
    processed_alpha = []
    target_w = int(WIDTH * 0.98)  # 1880px width (fills hero nicely)
    ...
    for name, fname in STATE_FILES:
        ...
        # Extract text mask from black background
        ...
        # Find tight bounding box of letters
        y_idx, x_idx = np.where(mask > 0.05)
        ...
        print(f"  Processed {name}: {new_w}x{new_h} centered in {WIDTH}x{HEIGHT}")
...
def render_white_hero_video():
    ...
    print(f"\nEncoding Web-Optimized Hero Video: {OUTPUT_MP4}")
    ffmpeg_cmd = [
        FFMPEG_EXE, '-y',
        '-f', 'rawvideo',
        '-vcodec', 'rawvideo',
    ...
    print(f"Rendering 1050 frames ({WIDTH}x{HEIGHT} on pure white background)...")
    ...
    for seg_idx, (f_start, f_end, s_from, s_to, mode) in enumerate(SEGMENTS):
        ...
        for f in range(f_start, f_end):
            ...
            if mode == "intro_glass":
                ...
                curr_alpha = alpha_a * (0.3 + 0.7 * fade)
                curr_rgb = rgb_a * fade
            ...
            # COMPOSITE ONTO PURE WHITE BACKGROUND (#ffffff)
            curr_alpha = np.clip(curr_alpha, 0.0, 1.0)[:, :, None]
            curr_rgb = np.clip(curr_rgb, 0.0, 1.0)
            composited = curr_rgb * curr_alpha + (1.0 - curr_alpha) * 1.0
    ...
    print("=" * 60)
    print("  WHITE HERO RENDER COMPLETE!")
