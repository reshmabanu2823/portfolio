...
for i, f in enumerate(files):
    src = os.path.join(brain_health_dir, f)
    im = Image.open(src)
    
    name_base = f"brain_health_{i+1}"
    
    # Save original jpg
    shutil.copyfile(src, os.path.join(work_dir, f"{name_base}.jpg"))
    shutil.copyfile(src, os.path.join(www_work_dir, f"{name_base}.jpg"))
    
    # Save WebP at 96% quality
    im.save(os.path.join(work_dir, f"{name_base}.webp"), 'WEBP', quality=96)
    im.save(os.path.join(www_work_dir, f"{name_base}.webp"), 'WEBP', quality=96)
    ...
    print(f"Saved {name_base} assets successfully!")

print("All Brain Health image assets processed!")
