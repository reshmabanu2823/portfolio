"""
=============================================================================
  HERO 16:9 THREE.JS REVEAL VIDEO RENDERER (WHITE STUDIO BACKGROUND)
=============================================================================
Renders 1920x1080 16:9 video with:
- Pure white background (#ffffff)
- Centered "RESHMA BANU" 3D morphing typography matching hero SVG layout
- 10 continuous material states (35s @ 30fps)
...
OUTPUT_MP4 = os.path.join(WORKSPACE, 'reshma_banu_hero_reveal_16_9.mp4')
...
# ROI for typography in 1080p
ROI_Y1, ROI_Y2 = 380, 700
ROI_X1, ROI_X2 = 120, 1800
ROI_H = ROI_Y2 - ROI_Y1
ROI_W = ROI_X2 - ROI_X1

def load_and_preprocess_states():
    print("Loading 10 states for 16:9 hero reveal...")
    processed_rgb = []
    processed_alpha = []
    target_w = int(WIDTH * 0.82)  # ~1574px width
    ...
    for name, fname in STATE_FILES:
        ...
        if 'glass' in name:
            mask = np.maximum(mask, np.clip(max_c * 1.8, 0, 1))

        y_idx, x_idx = np.where(mask > 0.04)
        ...
        full_rgb = np.zeros((HEIGHT, WIDTH, 3), dtype=np.float32)
        full_alpha = np.zeros((HEIGHT, WIDTH), dtype=np.float32)

        paste_x = (WIDTH - new_w) // 2
        paste_y = (HEIGHT - new_h) // 2

        full_rgb[paste_y:paste_y+new_h, paste_x:paste_x+new_w] = resized_rgb
        full_alpha[paste_y:paste_y+new_h, paste_x:paste_x+new_w] = resized_alpha

        roi_rgb = full_rgb[ROI_Y1:ROI_Y2, ROI_X1:ROI_X2].copy()
        roi_alpha = full_alpha[ROI_Y1:ROI_Y2, ROI_X1:ROI_X2].copy()

        processed_rgb.append(roi_rgb)
        processed_alpha.append(roi_alpha)
        print(f"  Processed {name}: ROI {ROI_W}x{ROI_H}")
...
def render_16_9_white_hero_video():
    ...
    # 1080p full frame buffer initialized to solid pure white
    full_frame = np.ones((HEIGHT, WIDTH, 3), dtype=np.uint8) * 255

    print(f"\nEncoding 16:9 Hero Video: {OUTPUT_MP4}")
    ffmpeg_cmd = [
        FFMPEG_EXE, '-y',
        '-f', 'rawvideo',
        '-vcodec', 'rawvideo',
    ...
    print(f"Rendering 1050 frames (1920x1080 with pure white background)...")
    ...
    for seg_idx, (f_start, f_end, s_from, s_to, mode) in enumerate(SEGMENTS):
        ...
        for f in range(f_start, f_end):
            ...
            if mode == "intro_glass":
                ...
                curr_rgb += glint[:, :, None] * curr_alpha[:, :, None] * 0.4 * (1.0 - local_t)
            ...
            if abs(float_y) > 0.05 or abs(float_x) > 0.05:
                M = np.float32([[1.0, 0, float_x],
                                [0, 1.0, float_y]])
                curr_rgb = cv2.warpAffine(curr_rgb, M, (ROI_W, ROI_H), borderMode=cv2.BORDER_CONSTANT, borderValue=0)
                curr_alpha = cv2.warpAffine(curr_alpha, M, (ROI_W, ROI_H), borderMode=cv2.BORDER_CONSTANT, borderValue=0)

            # Composite ROI onto pure white (1,1,1)
            curr_alpha = np.clip(curr_alpha, 0.0, 1.0)[:, :, None]
            curr_rgb = np.clip(curr_rgb, 0.0, 1.0)
            composited_roi = curr_rgb * curr_alpha + (1.0 - curr_alpha) * 1.0

            roi_uint8 = (np.clip(composited_roi, 0.0, 1.0) * 255.0).astype(np.uint8)
    ...
    print("=" * 60)
    print("  16:9 HERO RENDER COMPLETE!")
